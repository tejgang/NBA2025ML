'''
Monte Carlo random forrest game by game: with added variance (noisy stats)
Essentially the same as the original Monte Carlo Framework, however this adds variance within the statistics
'''
...
def noisy_stats(stats, std_dev=0.1):
    ...
    """
    seed = stats[0]
    noisy = [seed]  # seed stays unchanged

    for s in stats[1:]:
        if isinstance(s, (float, int)):
            noise = random.gauss(0, std_dev * abs(s))
            noisy.append(round(s + noise, 4))
        else:
            noisy.append(s)
...
def series_sim(team1, team2, team1_stats, team2_stats, model):
    ...
    while team1_wins < 4 and team2_wins < 4:
        # Introduce noise for each game
        team1_game_stats = noisy_stats(team1_stats)
        team2_game_stats = noisy_stats(team2_stats)

        winner = model(team1_game_stats, team2_game_stats)
