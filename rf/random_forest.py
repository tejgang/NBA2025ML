...
grid_search = GridSearchCV(
    ...
    cv=tscv,
    scoring='accuracy',  # Changed from roc_auc to accuracy to avoid warnings
    n_jobs=-1,
    verbose=1
)
...
if len(np.unique(y)) > 1:
    ...
    print("\nModel performance on training data:")
    print(f"Accuracy: {train_accuracy:.4f}")
    print("ROC AUC: Not applicable (only one class present)")
...
for test_year in range(2010, 2025):
    ...
    y_hist_test = hist_test['is_champion']
    ...
    # Train model
    hist_model = RandomForestClassifier(**grid_search.best_params_, random_state=42)
    ...
    preds = hist_model.predict(X_hist_test)
...
hist_df.to_csv('historical_prediction_results_rf.csv', index=False)
print("Historical results saved to 'historical_prediction_results_rf.csv'")
...
team_probs.to_csv('championship_predictions_2025.csv', index=False)
print("Predictions saved to 'championship_predictions_2025.csv'")
